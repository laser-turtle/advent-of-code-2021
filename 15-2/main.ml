...
let input =
    ...
    |> fun lines ->
        ...
        let map = Array.make_matrix ~dimx:(dimx*5) ~dimy:(dimy*5) 0 in
        ...
        for offx=0 to 4 do
            for offy=0 to 4 do
                if offx <> 0 || offy <> 0 then (
                    for x=0 to dimx-1 do
                        for y=0 to dimy-1 do
                            let ox = max 0 (offx-1) in
                            let oy = max 0 (offy-1) in
                            let above = map.(x+offx*dimx).(y+oy*dimy) in
                            let left  = map.(x+ox*dimx).(y+offy*dimy) in
                            let v = max left above + 1 in
                            map.(x+offx*dimx).(y+offy*dimy) <- if v > 9 then 1 else v;
                        done
                    done;
                )
            done
