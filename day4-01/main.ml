...
module Game : sig
    ...
    val find_winner : t -> (int * Board.t) option
end = struct
    type t = {
        boards : Board.t list;
        numbers : int list;
    ...
    let find_winner (t : t) : (int * Board.t) option =
        List.find_map t.numbers ~f:(fun num ->
            List.iter t.boards ~f:(Board.mark ~num);
            List.find_map t.boards ~f:(fun board ->
                match Board.is_winner board with
                | true -> Some (num, board)
                | false -> None
            )
        )
